...
const ConsultarSalidas = ({ data }) => {
  ...
  const handleShow = (row) => {
      ...
      console.log('Estos son mis datos:'+ JSON.stringify(row.idsalida))
      requestGet(`consultarSalidaDetallePorId&idSalida=`+row.idsalida)
        .then((res)=>{
          console.log('Entrada detalle: '+res)
          setDatos2(JSON.parse(res))
            ...
            >
                ...
                <Modal.Body>
                  <div ref={componentRef} className='container' style={{width: '100%', margin: '2px', position: 'center'}}>
                    <div className="card">
                      <div className="card-header">
                        <h4>Registro Salida de Movimiento</h4>
                      </div>
                    </div>
